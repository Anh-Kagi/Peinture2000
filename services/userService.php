...
class UserService{
    public static function getAllUsers(){
		...
		foreach(Database::query("SELECT id, name, hash FROM user") as $row) {
			$users[] = new User($row["id"], $row["name"], $row["hash"]);
		}
		return $users;
    }
    ...
    public static function getUser(string $name) {
        $params = array();
        $params[":name"] = $name;
        $row = Database::prepare("SELECT id, name, hash from user WHERE name = :name", $params);
        return new User($row[0]["id"], $row[0]["name"], $row[0]["hash"]);
    }
